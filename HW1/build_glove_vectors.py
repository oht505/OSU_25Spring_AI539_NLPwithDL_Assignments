...
def main_glove():
	...
	learningRate = 0.01	# learning rate / step size for SGD
	...
	for epoch in range(maxEpoch):
		...
		for b in range(idx.shape[0]//B+1):
			...
			wordvecs_grad = fval * 2 * error * c_batch
			contextvecs_grad = fval * 2 * error * w_batch
			wordbiases_grad = fval * 2 * error
			contextbiases_grad = fval * 2 * error
			########################################################################
